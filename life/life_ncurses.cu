// life_ncurses.cu — CUDA host + ncurses UI
...
int main(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        if (std::strcmp(argv[i], "--benchmark") == 0) benchmark_mode = true;
        // --cpu accepted for compatibility but ignored (CUDA only)
    }
    ...
    double prev_ts = benchmark_mode ? time_in_seconds() : 0.0;
    double bench_start = prev_ts;

    for (int step = 0; step < STEPS; ++step) {
        life_step_kernel<<<grid, block>>>(d_a, d_b, WIDTH, HEIGHT);
        check(cudaGetLastError(), "kernel launch");
        std::swap(d_a, d_b);

        const int* d_curr = d_a;
        check(cudaMemcpy(h_out, d_curr, BYTES, cudaMemcpyDeviceToHost), "D2H frame");
    ...
    if (benchmark_mode) {
        double bench_end = time_in_seconds();
        double elapsed = bench_end - bench_start;
        double fps = (elapsed > 0.0) ? (STEPS / elapsed) : 0.0;
        std::printf("Benchmark complete: %.2f seconds, %.2f FPS\n\n", elapsed, fps);
    }
